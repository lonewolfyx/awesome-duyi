...
function handleEffectiveVideo(channel: DouYinChannel, video: any) {
...
export async function getDouYinData(channel: DouYinChannel) {
    const query = {
    ...
    if (videos) {
        ...
        if (contentLastTime < lastTime) {
            ...
            oldContent.unshift({
                ...
                time: dayjs.unix(row.create_time).format('YYYY-MM-DD HH:mm:ss'),
